...
const generateDate = ({
  random,
  year = 2017,
  month = (rand: any) => Math.floor(rand() * 12),
  day = (rand: any) => Math.floor(rand() * 30) + 1,
}: any) => {
  const getPart = (part: any) =>
    typeof part === "function" ? part(random) : part;
...
export const globalSalesValues = {
  ...
  units: ({ random }: any) => Math.floor(random() * 4) + 1,
  customer: [
    "Renewable Supplies",
    "Energy Systems",
    "Environment Solar",
  ...
  amount: ({ random }: any) => Math.floor(random() * 1000000 + 1000) / 20,
  discount: ({ random }: any) => Math.round(random() * 0.5 * 1000) / 1000,
  saleDate: ({ random }: any) =>
    generateDate({
      random,
      year: 2016,
...
export const employeeTaskValues = {
  ...
  startDate: ({ random }: any) =>
    generateDate({
      random,
      year: 2016,
    }),
  dueDate: ({ random, record }: any) =>
    generateDate({
      random,
      year: 2016,
      month: () =>
